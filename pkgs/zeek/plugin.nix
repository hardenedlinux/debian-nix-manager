{ fetchFromGitHub, fetchgit, writeScript, postgresql, rdkafka, version, confdir, PostgresqlPlugin, KafkaPlugin, zeekctl, SpicyPlugin }:

rec {
  install_plugin = writeScript "install_plugin" (import ./install_plugin.nix { });
  spicy = fetchgit (builtins.fromJSON (builtins.readFile ./zeek-plugin.json)).spicy;
